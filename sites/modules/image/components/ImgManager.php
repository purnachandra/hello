...
class ImgManager extends CApplicationComponent
{
	 ...
	 */
	public function save($file,$name=null,$path=null,$id = null)
	{
		...
		try
		{
                       ...
                       //$uid = Yii::app()->user->id;
                       // $old = Image::model()->findByAttributes(array( 'ug_id'=>(int)$uid));
			...
			if($path!==null)
				$image->path=trim($path,'/');
                      // print_r($image->attributes);Yii::app()->end();
			//if(empty($image->id))
                      //  print_r($image->getErrors()); Yii::app()->end();
                            if($image->save()===false){
                                }

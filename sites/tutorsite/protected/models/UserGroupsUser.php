...
class UserGroupsUser extends CActiveRecord
{
	...
	private $_identity;
        ...
        public $terms_ok = 0;  /*  for terms condtions */
	/**
	...
	const REGISTRATION = 'registration';
        ...
        /**
         * Added by chandra for separate directories for different users
         */
	...
	public function rules()
	{
		...
		$rules = array(
			...
			// rules for multiple scenarios
                        array('group_id, group_name,terms_ok', 'required', 'on' => array( 'registration')),
			...
			array('password_confirm', 'required', 'on'=>array('registration', 'recovery','changePassword')),
                       array('terms_ok','compare','compareValue' => '1','on'=>array('registration'),'message'=>'You must aggree to the terms and conditions'),
			...
			array('id, group_name, group_id, username, home, status', 'safe', 'on'=>'search'),
                        array('facebook, firstname, lastname ,displayname, gender, group_id','safe','on'=>'registration'),
		...
		if (UserGroupsConfiguration::findRule('simple_password_reset') === false)
			array_push($rules, array('question, answer', 'required', 'on'=>array('recovery', //'registration',
                                                                                             'changePassword')));
	...
	public function relations()
	{
		...
		Yii::import('application.models.UserGroupsAccess');
		...
		$profiles = isset($modulesData['dashboard']['profile']) ? $modulesData['dashboard']['profile'] : array();
		// makes the relations
	...
	public function attributeLabels()
	{
		return array(
			'id' => 'ID',
			'group_id' => Yii::t('userGroupsModule.general','Account Type'),
	...
	{ 
		if (parent::beforeSave()) {
			...
			if ($this->scenario === 'registration') {
				if(empty($this->group_id)) $this->group_id = UserGroupsConfiguration::findRule('user_registration_group');
	...
	{   
		...
		{      
			if ($mode === 'regular') {
               ...
               // echo 'Error code:'.$this->_identity->errorCode;Yii::app()->end();
  ...
  public function findByEmail($email){     
   ...
   if(!empty($user))
       ...
       return new UserGroupsUser('insert');
//$criteria = new CDbCriteria();
//$criteria->condition = "MATCH (email) AGAINST ('$email' IN BOOLEAN MODE)";

//$user =  self::model()->findAll($criteria);

//echo $user->email; Yii::app()->end();
  }
        /*
         * End of By chandra 001
         */
}
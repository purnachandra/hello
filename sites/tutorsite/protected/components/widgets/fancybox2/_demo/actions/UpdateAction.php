   ...
   //! $model = $controller->loadModel();
    ...
    if( $this->isAjaxRequest )
    {
      ...
      echo CJSON::encode( array(
        ...
        'content' => $controller->renderPartial( $this->ajaxView, array(
          'model' => $model,
          'cancelUrl' => $this->getRedirectUrl( $model->id ) ), true, true ),
      ));
    ...
    // Render update page without using AJAX
      ...
      $controller->render( $this->view, array(
        'model' => $model,
        'cancelUrl' => $this->getRedirectUrl( $model->primaryKey ) ) );
  }
  ...
  protected function getRedirectUrl( $id )
  {
    ...
    if( $this->_redirectTo === null )
    {
      ...
      if( $this->redirectTo === null )
        $this->_redirectTo = array( '/dashboard/dash/myprofile'  );  //array( 'view', 'id' => $id );

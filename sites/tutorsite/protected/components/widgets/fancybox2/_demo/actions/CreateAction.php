...
class CreateAction extends CAction
{
  ...
  public function run()
  {
    ...
    // Get the Model class
    $modelClass = ucfirst( $controller->getId() ); // 'Message';
    
    // Create the model
    $model = new $modelClass();  //new Message();  //
   ...
   /*  $receiverName = Yii::app()->request->getPost('receiver');
       ...
       if(empty($model->sender_id)) $model->sender_id = 1;
                    if(empty($model->receiver_id)) $model->receiver_id = 1;*/

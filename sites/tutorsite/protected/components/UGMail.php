...
class UGMail {
	...
	protected function extractUserData(UserGroupsUser $model)
	{
		...
		$link = 'http://'.$_SERVER['HTTP_HOST'].Yii::app()->homeUrl.'user/activate';
...
class UGMailActivation implements UGMailMessage {
	...
	public function mailBody($data)
	{
		...
		return Yii::app()->controller->renderPartial('//mail/activation', array(
			'data' => $data
			), true);
	}
...
class UGMailPassReset implements UGMailMessage {
	...
	public function mailBody($data)
	{
		...
		return Yii::app()->controller->renderPartial('//mail/passreset', array(
			'data' => $data
			), true);
	}
...
class UGMailInvitation implements UGMailMessage {
	...
	public function mailBody($data)
	{
		return Yii::app()->controller->renderPartial('//mail/invitation', array(
			'data' => $data
			), true);
	}

<?php
Yii::import('application.components.UGCorn');
Yii::import('application.models.UserGroupsCron');
/**
...
class UserGroupsIdentity extends CUserIdentity
{
	...
	{        
		...
		//echo 'code:'.$this->errorCode;Yii::app()->end();
...
//var_dump('no error');Yii::app()->end();
	...
	{ 
		$array = array();
                ...
                $parray = array('Profile','Profiles');
                foreach ($parray as $p) {
		//foreach (Yii::app()->controller->getModule('userGroups')->profile as $p) {
	...
	public function getProfile()
	{
        ...
        public function social($email)
	{
		$user=UserGroupsUser::model()->findByEmail($email);
		if ( $user === null ) 
			$this->errorCode = self::ERROR_USERNAME_INVALID;
		else
		{

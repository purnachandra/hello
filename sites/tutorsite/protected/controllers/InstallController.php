...
class InstallController extends Controller
{
	...
	private function createController()
	{
		...
		$content = file_get_contents(Yii::app()->basePath . "/templates/template_Controller.php");
		if(@file_put_contents($path,$content)===false) {
			throw new CHttpException (500, Yii::t("userGroupsModule.install", "Unable to write the file {path}.", array("path"=>$path)));
			return false;
		}
	...
	private function createMailViews()
	{
		...
		$content = file_get_contents(Yii::app()->basePath . "/templates/template_activation.php");
		if(@file_put_contents($path,$content)===false) {
			throw new CHttpException (500, Yii::t("userGroupsModule.install", "Unable to write the file {path}.", array("path"=>$path)));
			return false;
		}
		...
		$content = file_get_contents(Yii::app()->basePath . "/templates/template_invitation.php");
		if(@file_put_contents($path,$content)===false) {
			throw new CHttpException (500, Yii::t("userGroupsModule.install", "Unable to write the file {path}.", array("path"=>$path)));
			return false;
		}
		...
		$content = file_get_contents(Yii::app()->basePath . "/templates/template_passreset.php");
		if(@file_put_contents($path,$content)===false) {
			throw new CHttpException (500, Yii::t("userGroupsModule.install", "Unable to write the file {path}.", array("path"=>$path)));
			return false;
		}
	...
	private function initializeConfiguration()
	{
		...
		$configuration_model->description = "password strength:<br/>weak: password of at least 5 characters, any character allowed.<br/>medium: password of at least 5 characters, must contain at least 2 digits and 2 letters.<br/>strong: password of at least 5 characters, must contain at least 2 digits, 2 letters and a special character.";
		if (!$configuration_model->save())
			throw new CHttpException (500, Yii::t("userGroupsModule.install", "Setting not installed. Installation abort."));
		$configuration_model = new UserGroupsConfiguration("installation");
	...
	private function initializeGroups()
	{
		...
		$group_model->home = "/dashboard/dash";
		$group_model->level = 1;
		$group_model->save();
	}
